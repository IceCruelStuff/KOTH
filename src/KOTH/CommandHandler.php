...
use KOTH\Events\ArenaCreateEvent;
use KOTH\Events\ArenaDeleteEvent;
...
class CommandHandler {
    ...
    public function handleCommand(CommandSender $sender, Command $command, /** @noinspection PhpUnusedParameterInspection */ string $label, array $args): bool {
        if ($command->getName() == "koth") { // Is this really done server side ?? (if i only register /koth ?)
            if ((!$sender instanceof Player) && $this->plugin->getServer()->getMotd() !== "Jacks-Test-Server"){  // To help me debug faster.
            ...
            switch ($args[0]) {
                case 'help':
                    ...
                    if ($sender->hasPermission("koth.join")) {
                        $sender->sendMessage(C::GOLD . "/koth join (arena name)" . C::RESET . " - Join a game.");
                    }
                    if ($sender->hasPermission("koth.leave")) {
                        $sender->sendMessage(C::GOLD . "/koth leave " . C::RESET . "- Leave a game you'r currently in.");
                    }
                    if ($sender->hasPermission("koth.start")) {
                        $sender->sendMessage(C::GOLD . "/koth start (arena name - optional) " . C::RESET . "- Starts a arena if game requirements are met.");
                    }
                    if ($sender->hasPermission("koth.forcestart")) {
                        $sender->sendMessage(C::GOLD . "/koth forcestart (arena name - optional) " . C::RESET . "- Forces a arena/game to start the countdown to begin.");
                    }
                    if ($sender->hasPermission("koth.new")) {
                        $sender->sendMessage(C::GOLD . "/koth new (arena name - no spaces) (min players) (max players) (gametime in seconds)" . C::RESET . " - Start the setup process of making a new arena.");
                    }
                    if ($sender->hasPermission("koth.rem")) {
                        $sender->sendMessage(C::GOLD . "/koth rem (arena name)" . C::RESET . " - Remove a area that has been setup.");
                    }
                    if ($sender->hasPermission("koth.setspawns")) {
                        $sender->sendMessage(C::GOLD . "/koth setspawn (arena name) " . C::RESET . "- Set a spawn point for a arena.");
                    }
                    if ($sender->hasPermission("koth.setpos")) {
                        $sender->sendMessage(C::GOLD . "/koth setpos1 (arena name) or /koth setpos2 (arena name> " . C::RESET . "- Set king area corner to corner.");
                    }
                    if ($sender->hasPermission("koth.addrewards")) {
                        $sender->sendMessage(C::GOLD . "/koth addreward (arena name) (command eg. /give {PLAYER} 20 1)" . C::RESET . " - Add a command to execute when winner is announced");
                    }
                    return true;
                case 'credits':
                ...
                case 'start':
                    ...
                    if ((count($args) < 2) && $this->plugin->getArenaByPlayer($sender->getLowerCaseName()) === null) {
                ...
                case 'addreward':
                    ...
                    unset($args[0]);
                    unset($args[1]);
                    $command = array_values($args);
                    $arena->rewards[] = implode(" ", $command);
    ...
    private function listArenas(CommandSender $sender): void {
        ...
        if (count($list) === 0) {
            $sender->sendMessage($this->prefix . C::RED . "There are no arenas");
            return;
        }
    ...
    private function createArena(CommandSender $sender, array $args): void {
        ...
        $arena = new Arena(
            ...
            $event->getMaxPlayers(),
            $event->getGameTime(),
            $event->getHillPositions(),
            $event->getSpawnPositions(),
            $event->getRewards(),
            $event->getWorld()
        );
        ...
        $sender->sendMessage(
            $this->prefix . C::GREEN . "Nice one, " . $name . " arena is almost fully setup, to complete the arena setup be sure to do '/koth setpos1 (arena name)' when standing on pos 1, and '/koth setpos2 (arena name)' when standing in the opposite corner."
        );
